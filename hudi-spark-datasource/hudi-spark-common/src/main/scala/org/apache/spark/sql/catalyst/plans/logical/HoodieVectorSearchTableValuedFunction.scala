...
import org.apache.spark.sql.catalyst.plans.logical.HoodieVectorSearchTableValuedFunction.{parseK, parseOptionalDouble, parseOptionalString}
...
object HoodieVectorSearchTableValuedFunction {
   ...
   *   max_distance excludes results beyond the given distance threshold.
   */
  def parseArgs(exprs: Seq[Expression]): ParsedArgs = {
    ...
    ParsedArgs(tableName, embeddingCol, queryVectorExpr, k, metric, algorithm, filter, maxDistance)
  }
...
object HoodieVectorSearchBatchTableValuedFunction {
  ...
  def parseArgs(exprs: Seq[Expression]): ParsedArgs = {
    ...
    val k = parseK(FUNC_NAME, exprs(4))
    ...
    val filter = if (exprs.size >= 8) parseOptionalString(FUNC_NAME, exprs(7), "filter") else None
    val maxDistance = if (exprs.size >= 9) parseOptionalDouble(FUNC_NAME, exprs(8), "max_distance") else None
    ParsedArgs(corpusTable, corpusEmbeddingCol, queryTable, queryEmbeddingCol, k, metric, algorithm, filter, maxDistance)
  }
}
