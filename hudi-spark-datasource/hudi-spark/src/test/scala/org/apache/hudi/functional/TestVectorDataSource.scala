...
class TestVectorDataSource extends HoodieSparkClientTestBase {
  ...
  def testVectorRoundTrip(): Unit = {
    ...
    assertTrue(readMetadata.contains("hoodie.vector.dimension"))
    assertEquals(128L, readMetadata.getLong("hoodie.vector.dimension"))
  ...
  def testNullableVectorField(): Unit = {
    ...
    val metadata = new MetadataBuilder()
      .putLong("hoodie.vector.dimension", 32)
      .build()
