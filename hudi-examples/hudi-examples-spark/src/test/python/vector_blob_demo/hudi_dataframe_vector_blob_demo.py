...
def build_target_schema(embedding_dim: int) -> StructType:
    ...
    the Hudi write — no Parquet round-trip to flip them back, no analyzer cast
    to fight.
    """
    return StructType(
        [
...
def write_to_hudi(spark, data, embedding_dim: int):
    ...
    a workaround for a different problem (macOS PythonRDD socket exhaustion at
    high N during sustained `df.write.parquet` streaming). It introduced a
    Parquet round-trip that silently reset `containsNull=True` and forced the
    SQL DDL workaround. `spark.createDataFrame(rows, schema)` is a one-shot
    serialization at construction — no sustained streaming, no socket pressure
    at demo size.
    """
    print(
